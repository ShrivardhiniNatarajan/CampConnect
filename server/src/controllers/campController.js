...
const getMyCamps = async (req, res) => {
  try {
    const [camps] = await pool.query(
      "SELECT c.camp_id, c.ticket_id, c.start_date, c.end_date, c.status AS camp_status, t.health_focus_area, t.expected_patient_count, t.pincode, t.status AS ticket_status, cr.report_id, cr.verified, cr.patients_served_count AS reported_patients, cr.proof_document_url FROM camps c INNER JOIN organizations o ON o.org_id = c.org_id INNER JOIN tickets t ON t.ticket_id = c.ticket_id LEFT JOIN camp_reports cr ON cr.camp_id = c.camp_id WHERE o.org_admin_id = ? ORDER BY c.created_at DESC",
      [req.user.user_id]
    );
